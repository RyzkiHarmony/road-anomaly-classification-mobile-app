...
interface TripDao {
    ...
    // AnomalyEvent methods
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertAnomalyEvent(event: AnomalyEvent)

    @Query("SELECT * FROM anomaly_events WHERE tripId = :tripId ORDER BY timestamp ASC")
    suspend fun getAnomalyEventsForTrip(tripId: String): List<AnomalyEvent>
}
